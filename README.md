...
将 [terrain-diffusion](https://huggingface.co/xandergos/terrain-diffusion-30m) 扩散模型封装为一条命令：给定世界坐标区域，输出 6 通道 32-bit float TIFF（高程 + 气候），可直接作为 GPU 纹理（R32Float）在 ComputeShader 中采样。

本项目聚焦**部署、使用、工程化**——不做模型研究，上游推理管线以最小修改嵌入 `vendor/`（改动见 `vendor/README.md`）。

## 环境

- Python ≥ 3.12，[uv](https://docs.astral.sh/uv/)，CUDA GPU（CPU 可跑，但慢约 10×）
...
# OpenEXR 输出（F32, 通道 'R'）
uv run python terrain_export.py --i1 0 --j1 0 --i2 256 --j2 256 --format exr -o out.exr
```

`-o out.tif` → `output/out/out_elev.tif` 等（`-o output.tif` 则落到 `output/` 根）。`--format` 支持 `tif`（默认）或 `exr`，均为 32-bit float 单通道，数值完全等价。完整参数见 `--help`。
...
- 全局像素坐标，原点 `(0,0)` 在左上，`i` 向南、`j` 向东，**30 m/像素**
- `(0,0) → (256,256)` = 7.68 km × 7.68 km；任意大小一次生成，无需分块
- 相同 `--seed` + 相同坐标 ⇒ 完全相同的地形

## 首次运行

1. 模型权重自动从 HuggingFace 拉取（默认镜像 `hf-mirror`，见 `terrain_export.py` 顶部 `HF_ENDPOINT`）
...
- 气候 4 通道（t_season / precip / p_cv / beta）来自约 7.7 km 分辨率 coarse map 的上采样，代表大尺度气候先验。已将上采样插值由双线性改为**双三次**（vendor 内唯一一处对上游的修改，见 `vendor/README.md`），消除粗单元边界的网格纹；`elev` / `temp` 经完整扩散管线，本无网格
